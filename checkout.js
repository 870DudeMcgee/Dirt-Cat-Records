const CHECKOUT_SERVICES = {
  mix: {
    ...
    priceCents: 14900,
    description:
      "Full stereo mix with Dirt Cat analog character and modern translation.",
  },
  master: {
    id: "master",
    label: "Master Only",
...
function getSelectedAddOn(addOnId) {
  return checkoutState.selectedAddOns.find(
...
function calculateClientEstimate() {
  ...
  const amountDueNowCents =
    checkoutState.paymentMode === "deposit"
...
function renderServices() {
  ...
  container.innerHTML = Object.values(CHECKOUT_SERVICES)
    .map(
      (service) => `
...
function renderAddOns() {
  ...
  if (service.depositOnly) {
    container.innerHTML =
      '<p class="field-help">Add-ons are scoped separately for custom projects. Use this deposit only after the project is quoted.</p>';
    return;
  }

  container.innerHTML = Object.values(CHECKOUT_ADDONS)
    .map((addOn) => {
      const selected = getSelectedAddOn(addOn.id);
      const billingLabel =
        addOn.billing === "perSong" ? "per song" : "per project";
      const quantityDisabled =
        addOn.billing === "perProject" || !selected ? "disabled" : "";
...
function renderSummary() {
  ...
  if (estimate.discountCents > 0) {
    lines.push(
      `<div class="summary-discount"><span>${estimate.discountPercent}% multi-song discount</span><strong>-${formatMoney(estimate.discountCents)}</strong></div>`
    );
  }

  estimate.addOnLines.forEach((line) => {
    const unitLabel =
      line.billing === "perSong"
        ? `${line.quantity} × ${estimate.songCount} song${estimate.songCount === 1 ? "" : "s"}`
        : "project add-on";
    lines.push(
      `<div><span>${line.label} <small>${unitLabel}</small></span><strong>${formatMoney(line.totalCents)}</strong></div>`
    );
  });

  if (checkoutState.discountApplied) {
    lines.push(
      `<div class="summary-discount"><span>Friends comp</span><strong>-${formatMoney(estimate.totalCents)}</strong></div>`
    );
  }
  ...
  lines.push(
    `<div class="summary-total"><span>Total</span><strong>${formatMoney(estimate.totalCents)}</strong></div>`
  );
  lines.push(
    `<div class="summary-total due-now"><span>Due now</span><strong>${formatMoney(displayedDueNowCents)}</strong></div>`
  );

  if (displayedRemainingBalanceCents > 0) {
    lines.push(
      `<div><span>Remaining balance</span><strong>${formatMoney(displayedRemainingBalanceCents)}</strong></div>`
    );
  }
  ...
  document.getElementById("no-charge-checkout").hidden =
    !checkoutState.discountApplied;
  document.getElementById("paypal-button-container").hidden =
    checkoutState.discountApplied;
}

function setAddOn(addOnId, enabled) {
  checkoutState.selectedAddOns = checkoutState.selectedAddOns.filter(
    (entry) => entry.addOnId !== addOnId
  );
  if (enabled) {
...
function buildClientOrderSummary() {
  ...
  return {
    ...
    discountedServiceSubtotalCents:
      estimate.serviceSubtotalCents - estimate.discountCents,
    addOnLineItems: estimate.addOnLines.map((line) => ({
      ...
      unitPriceCents: line.billedUnits
        ? Math.round(line.totalCents / line.billedUnits)
        : 0,
...
function assertFormReady() {
  ...
  if (!form.reportValidity()) {
    error.textContent =
      "Please complete the required checkout details before payment.";
...
function getCheckoutConfig() {
  if (!checkoutConfigPromise) {
    checkoutConfigPromise = fetch("/api/checkout-config").then(
      async (response) => {
        const config = await response.json();
        if (!response.ok) {
          throw new Error(
            config.error || "Checkout configuration is unavailable."
          );
        }
        return config;
      }
...
async function renderPayPalButtons() {
  ...
  buttonContainer.innerHTML =
    '<p class="field-help">Loading secure PayPal checkout…</p>';
    ...
    .Buttons({
      ...
      async onApprove(data) {
        ...
        const pending = JSON.parse(
          sessionStorage.getItem("dirtCatPendingOrder") || "{}"
        );
        sessionStorage.setItem(
          "dirtCatPaidOrder",
          JSON.stringify({
...
function renderLocalTestCheckout(config) {
  ...
  container.innerHTML =
    '<button type="button" class="btn btn-secondary" id="run-no-charge-test">Run No-Charge Test</button><p class="field-help">Local only. Skips PayPal and verifies the success flow without charging money.</p>';
  document
    .getElementById("run-no-charge-test")
    ?.addEventListener("click", () => {
      try {
        ...
        document.getElementById("checkout-error").textContent =
          error.message || "Unable to run no-charge test checkout.";
      }
    });
}
...
function wireEvents() {
  ...
  document.getElementById("song-count").addEventListener("input", (event) => {
    checkoutState.songCount = Math.max(
      1,
  ...
  document
    ...
    .addEventListener("change", (event) => {
      ...
      if (event.target.matches("[data-addon-quantity]")) {
        setAddOnQuantity(
          event.target.dataset.addonQuantity,
          event.target.value
        );
      }
    });
  ...
  document
    .getElementById("apply-discount-code")
  ...
  document
    .getElementById("no-charge-checkout")
    .addEventListener("click", submitNoChargeCheckout);
}
