...
- Mobile logo sizing and spacing have been tightened so the floating header no longer clips or crowds the checkout-style hero logos on customer-facing pages. The homepage phone hero now uses an inline logo, smaller heading scale, compact CTA stack, and safer `100svh` layout so the first viewport reads as a coherent offer instead of a pile-up.
