...
- Shared form CSS now uses border-box sizing and shrink-safe grid controls so checkout, portal, and support inputs/buttons do not overflow their glass panels. The support page also has its own shrink-safe field grid and action-button layout so narrow screens do not crowd the card.
