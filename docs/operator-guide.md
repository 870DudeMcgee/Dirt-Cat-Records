# Dirt Cat Records Operator Guide

This guide owns the detailed setup, runtime, credential, launch, and owner-proof instructions for Dirt Cat Records. Keep the top-level `README.md` focused on what the repo is and where to find deeper docs.

The product itself is a static marketing site, PayPal checkout, Supabase-backed customer portal, Google Drive project folder automation, Resend transactional email, and a private owner admin dashboard for Dirt Cat Records.
...
The staged task list lives in [`roadmap.md`](roadmap.md).

The current priority is Stage 7: launch hardening across the live providers and production configuration.
Use [`workflow.md`](workflow.md) and [`deployment-ledger.md`](deployment-ledger.md) before any shared preview or production retest, and use the remaining workflow hardening tasks in [`superpowers/plans/2026-05-20-workflow-and-version-control-hardening-plan.md`](superpowers/plans/2026-05-20-workflow-and-version-control-hardening-plan.md) for the follow-on cleanup.
The ordered follow-on plan for the PayPal environment and webhook seams lives in [`superpowers/plans/2026-05-20-paypal-environment-deepening-plan.md`](superpowers/plans/2026-05-20-paypal-environment-deepening-plan.md).
The ordered follow-on plan for workflow deepening after the PayPal seam work lives in [`superpowers/plans/2026-05-20-deep-modules-architecture-plan.md`](superpowers/plans/2026-05-20-deep-modules-architecture-plan.md).
The durable architecture gap register lives in [`superpowers/specs/2026-05-20-architecture-readiness-review.md`](superpowers/specs/2026-05-20-architecture-readiness-review.md).
Future product exploration for the Logic Pro stem exporter desktop app lives in [`superpowers/specs/2026-05-21-logic-stem-exporter-deep-modules.md`](superpowers/specs/2026-05-21-logic-stem-exporter-deep-modules.md). It is intentionally separate from the active web checkout, portal, and admin launch-hardening work.
The executable follow-on implementation plan for that separate desktop-app repo lives in [`superpowers/plans/2026-05-21-logic-stem-exporter-implementation-plan.md`](superpowers/plans/2026-05-21-logic-stem-exporter-implementation-plan.md).
Future growth-tool and lead-magnet exploration lives in [`superpowers/specs/2026-05-21-dirt-cat-growth-tools-brainstorm.md`](superpowers/specs/2026-05-21-dirt-cat-growth-tools-brainstorm.md). It is also future product work, not part of the active launch-hardening slice.
...
The standard start-work flow lives in [`workflow.md`](workflow.md).

## Deployment Provenance

Shared preview and production deployments must be tied to a pushed commit and recorded in [`deployment-ledger.md`](deployment-ledger.md).
...
- shared preview: team-facing preview deployment tied to a pushed SHA, recorded in [`deployment-ledger.md`](deployment-ledger.md), and eligible for the stable preview alias.
...
- Primary pages now expose Portal as a standard navigation link and use a shared responsive hamburger menu on narrow screens so the customer nav stays usable without wrapping across the fixed header; the mobile trigger now sits as a lighter left-side floating control instead of a heavy full-width black bar.
- Mobile hero logo sizing and spacing now leave room below the floating nav and keep the checkout, success, and support logos from feeling clipped or cramped at the top of the viewport.
- The homepage phone hero now follows the checkout page's roomier mobile rhythm: the inline logo, headline, subtitle, and CTAs scale up for better presence while still fitting the first iPhone viewport, and the opened hamburger menu uses a vertical neon signal list so navigation does not bury the offer.
...
- `README.md`: GitHub-facing project overview, current status summary, quick start, and doc map.
- `docs/operator-guide.md`: operator workflow, setup requirements, runtime commands, credential checklist, launch checklist, and owner-proof instructions.
...
- [`execution-trail.md`](execution-trail.md) for the required process.
- [`execution-log.md`](execution-log.md) as the append-only working log.
...
Deployment triage notes live in [`deployment-preflight.md`](deployment-preflight.md).
