...
test("createTestRunId creates traceable run ids", () => {
  assert.match(
    createTestRunId("simulation"),
    /^simulation-\d{8}T\d{6}-[a-z0-9]{6}$/
  );
});
...
function defaultProviders() {
  return {
    ...
    storage: {
      check: async () => ({
        status: "skipped",
        detail: "Drive live check is run by sandbox mode.",
      }),
    },
    email: {
      check: async () => ({
        status: "skipped",
        detail: "Email live check is run by sandbox mode.",
      }),
    },
    payments: {
      check: async () => ({
...
async function cleanupAutomationTestRun({
  ...
  for (const record of report.createdRecords || []) {
    if (
      ...
      !record.id ||
      typeof records.updateProject !== "function"
    )
      continue;
    try {
...
const {
  createCleanupTestRunHandler,
} = require("../api/admin/cleanup-test-run");
...
test("cleanup endpoint cleans a stored test run for admin", async () => {
  const handler = createCleanupTestRunHandler({
    ...
    cleanupAutomationTestRunImpl: async ({ report }) => ({
      ...report,
...
function createTestRunsHandler(dependencies = {}) {
  ...
  const runAutomationTestImpl =
    dependencies.runAutomationTestImpl || runAutomationTest;
  const env = dependencies.env || process.env;
...
function createCleanupTestRunHandler(dependencies = {}) {
  ...
  const cleanupAutomationTestRunImpl =
    dependencies.cleanupAutomationTestRunImpl || cleanupAutomationTestRun;
  const env = dependencies.env || process.env;
  ...
  <body>
    <main class="admin-shell">
      <section class="admin-header">
        ...
        <button class="btn btn-secondary" id="admin-refresh" type="button">
          Refresh
        </button>
      </section>
      ...
      <section class="setup-actions">
        <button class="btn" id="run-simulation" type="button">
          Run Simulation
        </button>
        <button class="btn btn-secondary" id="run-sandbox" type="button">
          Run Sandbox
        </button>
        <button
...
(function () {
  ...
  function bindActions() {
    document
      ...
      ?.addEventListener("click", loadSetup);
    document
