...
function calculateOrder(input) {
  ...
  const service = getCatalogItem(SERVICES, orderInput.baseServiceId, 'base service');
  ...
  if (service.depositOnly && paymentMode !== 'full') {
    throw new Error('Custom project deposit is already a deposit and must be paid in full.');
  }
