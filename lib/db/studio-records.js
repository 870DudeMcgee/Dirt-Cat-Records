...
const { selectFollowUps } = require("../automation/follow-up-selector");
...
async function getFollowUpCandidates(options = {}) {
  ...
  return selectFollowUps(projects, {
    now: options.now,
...
async function queueFollowUpJobs(candidates, options = {}) {
  ...
  for (const candidate of candidates) {
    ...
    const followUpType = candidate?.reminderType;

    if (!projectId || !customerId || !followUpType) {
      skipped.push({
        projectId: projectId || null,
        reminderType: followUpType || null,
    ...
    try {
      ...
      await logFollowUpEvent(
        {
          projectId,
          eventType: "followup_job_enqueued",
      ...
      if (isDuplicateFollowUpPendingError(error)) {
        skipped.push({
          projectId,
          reminderType: followUpType,
        ...
        await logFollowUpEvent(
          {
            projectId,
            eventType: "followup_job_duplicate_skipped",
            message: `Skipped duplicate pending ${followUpType} follow-up job.`,
            metadata: { reminderType: followUpType },
          },
          queryOptions
        );
        continue;
      ...
      await logFollowUpEvent(
        {
          projectId,
          eventType: "followup_job_enqueue_failed",
          message: `Failed to queue ${followUpType} follow-up job.`,
          metadata: { reminderType: followUpType, error: error.message },
        },
        queryOptions
      );
    }
...
function isDuplicateFollowUpPendingError(error) {
...
async function updateAdminProjectDelivery(
  ...
  const eventType = shouldUnlock
    ? "final_delivery_unlocked"
    : "admin_delivery_updated";
        ...
        const result = await sendEmail(
          {
            ...
            data: {
              balanceUrl: buildPortalUrl(options.env),
