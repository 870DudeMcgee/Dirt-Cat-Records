...
async function getQuoteForProjectCustomer({ quoteId, projectId, customerId }, options = {}) {
  const rows = await supabaseRequest('/quotes', {
    query: {
      id: `eq.${requireValue(quoteId, 'quote id')}`,
...
async function sendAdminQuote(projectId, quoteId, options = {}) {
  ...
  const customer = project.customers || {};
  if (!customer.email) {
    const error = new Error('Project customer email is required to send a quote.');
    error.statusCode = 400;
    throw error;
  }
  ...
  const totalLabel = formatCentsToDollars(
    sentQuote.final_total_cents !== undefined ? sentQuote.final_total_cents : quote.final_total_cents || 0
  );
...
function normalizeAdminQuoteInput(input = {}) {
  ...
  const catalogTotalCents = normalizeCents(input.catalogTotalCents, 'catalogTotalCents');
  const adjustmentCents = normalizeSignedCents(input.adjustmentCents || 0, 'adjustmentCents');
  ...
  if (paymentMode === 'deposit') {
    ...
    if (depositCents < 1 || depositCents > finalTotalCents) {
      const error = new Error('depositCents must be between 1 and final total.');
      error.statusCode = 400;
      throw error;
    }
...
function buildQuoteFromCatalogInput(input = {}) {
  ...
  const lineItems = [
    {
      ...
      unitCents: Math.round(Number(orderSummary.serviceSubtotalCents || 0) / Number(orderSummary.songCount || 1)),
  ...
  if (adjustmentCents !== 0) {
    lineItems.push({
      itemType: 'adjustment',
      itemId: null,
      label: (typeof input.adjustmentLabel === 'string' && input.adjustmentLabel.trim())
...
function normalizeAdminQuoteLineItems(items) {
  ...
  return items.map((item, index) => {
    const itemType = requireValue(item.itemType, `lineItems[${index}].itemType`);
    ...
    if (!Number.isInteger(quantity) || quantity < 1) {
      const error = new Error(`lineItems[${index}].quantity must be a positive integer.`);
      error.statusCode = 400;
      throw error;
    }
    const unitCents = normalizeSignedCents(item.unitCents, `lineItems[${index}].unitCents`);
    if (itemType !== 'adjustment' && unitCents < 0) {
      const error = new Error(`lineItems[${index}].unitCents must be non-negative for service/add_on.`);
      error.statusCode = 400;
      throw error;
    }
    return {
...
function normalizeAdminQuoteLineItem(item = {}) {
  return {
    ...
    unitCents: Number(item.unit_cents !== undefined ? item.unit_cents : item.unitCents || 0),
    totalCents: Number(item.total_cents !== undefined ? item.total_cents : item.totalCents || 0),
  };
}
...
function buildQuoteUrl(projectId, quoteId, env = process.env) {
  const base = (env && env.SITE_URL ? env.SITE_URL : '').replace(/\/$/, '');
