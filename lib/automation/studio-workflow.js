...
function createFreeReviewWorkflow(dependencies = {}) {
  ...
  const email = dependencies.email || createEmailAdapter(dependencies.resend || resendDefault, records, dependencies.env);
...
function createPaidProjectWorkflow(dependencies = {}) {
  ...
  return async function paidProjectWorkflow(input) {
    ...
    if (input.paymentPurpose === 'quote' && input.quoteId && input.projectId) {
      const converted = await handleQuotePayment({
        records,
        customer,
        orderPayment,
        input,
    ...
    const existingProject = await findExistingPaidProject(records, orderPayment);
    if (existingProject) {
      return { customer, order: orderPayment.order, payment: orderPayment.payment, project: existingProject };
    }
    ...
    try {
      ...
      const projectCreatedConcurrently = /projects_order_id|duplicate key|unique/i.test(error.message || '')
        ? await findExistingPaidProject(records, orderPayment)
        : null;
      if (!projectCreatedConcurrently) throw error;
      return { customer, order: orderPayment.order, payment: orderPayment.payment, project: projectCreatedConcurrently };
    }
    ...
    if (records.linkOrderPaymentToProject) {
      await records.linkOrderPaymentToProject({
        orderId: orderPayment.order.id,
...
async function handleQuotePayment({ records, customer, orderPayment, input }) {
  const quote = records.getQuoteById ? await records.getQuoteById(input.quoteId) : null;
  if (!quote) throw new Error('Quote payment referenced an unknown quote.');
  const project = records.getProjectById ? await records.getProjectById(input.projectId) : null;
  ...
  const totalAmount = Number(input.totalAmount || Number(quote.final_total_cents || 0) / 100);
  const balanceDue = input.remainingBalance !== undefined
  ...
  const updatedProject = await records.updateProject(project.id, {
    ...
    final_delivery_locked: balanceDue > 0,
    active_quote_id: quote.id,
  });

  if (records.updateQuote) {
...
async function sendAndLogEmail({ resend, records, to, emailType, data, env }) {
  try {
    const result = await resend.sendStudioEmail({ to, emailType, data }, { env });
