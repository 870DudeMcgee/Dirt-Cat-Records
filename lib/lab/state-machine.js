(function initStateMachine(globalScope) {
  ...
  function getInitialState() {
    const defaultDataState = data ? data.DEFAULT_STATE : {
    ...
    switch (action.type) {
      ...
      case "SET_ARCHETYPE": {
        ...
        if (data && Array.isArray(data.ARCHETYPES)) {
          matchingArch = data.ARCHETYPES.find(arch => arch.id === archetypeId);
        }
      ...
      case "TOGGLE_PARAMETER_RUNG": {
        ...
        const currentValues = state.parameterSelections?.[parameterId] || [];
