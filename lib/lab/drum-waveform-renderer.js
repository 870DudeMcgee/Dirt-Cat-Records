(function initDrumWaveformRenderer(globalScope) {
    ...
    const startSample =
      options.startSample !== undefined
        ...
        : Number.isFinite(focusSample)
          ? focusSample - windowSamples * 0.35
          : 0;
  ...
  function drawLane(context, lane, rect, options) {
    ...
    drawGrid(context, contentRect, colors);
    ...
    if (lane.errorMessage) {
      ...
      strokePathCommands(context, beforeCommands, colors.before, 1.2);
      strokePathCommands(context, afterCommands, colors.after, 1.7);
    }
  ...
  function drawGrid(context, rect, colors) {
    ...
    for (let step = 1; step < 4; step += 1) {
      const x = rect.x + (rect.width * step) / 4;
  ...
  function createWaveformPath(samples, rect, options = {}) {
    ...
    for (let column = 0; column < columnCount; column += 1) {
      ...
      commands.push({ type: "moveTo", x, y: yMin });
      commands.push({ type: "lineTo", x, y: yMax });
    }
  ...
  function drawMarker(context, sample, rect, viewport, options) {
    ...
    drawText(context, options.label, x + 5, rect.y + 10, {
      color: options.color,
