...
async function getPayPalAccessToken({ fetchImpl = fetch, env = process.env } = {}) {
  ...
  if (!response.ok || !body.access_token) {
    const error = new Error(`Unable to get PayPal access token: ${formatResponseError(response, body)}`);
    error.diagnostics = getPayPalCredentialDiagnostics(env);
    throw error;
  }
...
function parseCompletedPaymentEvent(webhookEvent) {
  ...
  const orderSummary = getOrderSummaryFromResource(resource);
  ...
  return {
    ...
    status: 'paid',
    totalAmount: orderSummary ? centsToDollars(orderSummary.totalCents) : amountDueNow,
    amountDueNow,
    remainingBalance: orderSummary ? centsToDollars(orderSummary.remainingBalanceCents) : '0.00',
    currencyCode: amount.currency_code,
    orderSummary,
    paymentPurpose: orderSummary?.paymentPurpose || 'checkout',
    quoteId: orderSummary?.quoteId || null,
    projectId: orderSummary?.projectId || null,
    rawPayload: webhookEvent,
  };
}

function getOrderSummaryFromResource(resource) {
  const customId = resource.purchase_units?.find((unit) => typeof unit.custom_id === 'string')?.custom_id;
  if (!customId) return null;
  try {
    const metadata = parseOrderMetadata(customId);
    if (metadata.paymentPurpose === 'quote') {
      return {
        ...
        remainingBalanceCents: Math.max(0, metadata.totalCents - metadata.amountCents),
        paymentMode: 'full',
      };
    }
    return calculateOrder(metadata);
  } catch (error) {
    throw new Error(`Completed PayPal event has invalid checkout metadata: ${error.message}`);
  }
}
