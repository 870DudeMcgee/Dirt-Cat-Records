...
function getDriveConfig(env = process.env) {
  const config = {
    ...
    shareTestEmail: env.GOOGLE_DRIVE_TEST_SHARE_EMAIL || null,
  };
  if (
    !config.clientId ||
    !config.clientSecret ||
...
async function verifyDriveAccess(options = {}) {
  ...
  const url = new URL(
    `${DRIVE_API_BASE_URL}/files/${encodeURIComponent(config.projectsFolderId)}`
  );
  ...
  if (!response.ok || !body.id) {
    throw new Error(
      `Unable to access Google Drive projects folder: ${body.error?.message || response.status}`
    );
  }
...
async function createDriveProjectFolders(project, options = {}) {
  ...
  const shareEmail = resolveDriveShareEmail(project.customerEmail, env, config);
  if (shareEmail) {
    shareResult = await shareFolderWithEmail({
      accessToken,
      fetchImpl,
  ...
  return {
    ...
    uploadFolderShareSkippedReason: shareResult?.skippedReason || null,
    uploadFolderSharedWithEmail: shareEmail,
  };
}

function resolveDriveShareEmail(
  customerEmail,
  env = process.env,
  config = getDriveConfig(env)
) {
  const normalizedCustomerEmail = normalizeEmail(customerEmail);
  const normalizedOverrideEmail = normalizeEmail(config.shareTestEmail);

  if (normalizedOverrideEmail && env.PAYPAL_ENV !== "live") {
    return normalizedOverrideEmail;
  }
...
async function getAccessToken(config, fetchImpl) {
  ...
  if (!response.ok || !body.access_token) {
    throw new Error(
      `Unable to refresh Google access token: ${body.error || response.status}`
    );
  }
...
async function createFolder({ accessToken, fetchImpl, name, parentId }) {
  ...
  if (!response.ok || !body.id) {
    throw new Error(
      `Unable to create Google Drive folder: ${body.error?.message || response.status}`
    );
  }
  return body;
}
...
async function findFolder({ accessToken, fetchImpl, name, parentId }) {
  ...
  if (!response.ok) {
    throw new Error(
      `Unable to search Google Drive folders: ${body.error?.message || response.status}`
    );
  }
...
async function shareFolderWithEmail({
  ...
  const url = new URL(
    `${DRIVE_API_BASE_URL}/files/${encodeURIComponent(folderId)}/permissions`
  );
  ...
  const body = await readJson(response);
  if (!response.ok) {
    const shareErrorMessage = body.error?.message || String(response.status);
    if (isNonGoogleAccountShareError(shareErrorMessage)) {
      return {
        skippedReason: shareErrorMessage,
      };
    }
    throw new Error(
      `Unable to share Google Drive upload folder: ${shareErrorMessage}`
    );
  }
  return body;
}

function isNonGoogleAccountShareError(message) {
  return /do not have a Google Account/i.test(String(message || ""));
}
