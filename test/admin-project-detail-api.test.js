...
test('buildAdminProjectDetail normalizes project detail records', () => {
  const detail = buildAdminProjectDetail({
    ...
    adminNotes: [
      { id: 'note-1', note: 'Client wants brighter vocals.', created_at: '2026-05-18T09:45:00.000Z' },
    ],
  });
...
test('getAdminProjectDetail fetches project and related records', async () => {
  ...
  const detail = await getAdminProjectDetail('project-1', {
    ...
    fetchImpl: async (url) => {
      ...
      if (String(url).includes('/admin_notes')) return jsonResponse([{ id: 'note-1', note: 'Private note', created_at: '2026-05-19T09:00:00.000Z' }]);
  ...
  assert.ok(calls.some((url) => url.includes('/admin_notes?project_id=eq.project-1')));
});
...
test('updateAdminProjectStatus updates the project and logs an admin event', async () => {
  ...
  const detail = await updateAdminProjectStatus('project-1', 'mixing', {
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (String(url).includes('/projects?id=eq.project-1') && (options.method || 'GET') === 'GET') {
        return jsonResponse([{
          id: 'project-1',
          project_code: 'DCR-000123',
      ...
      if (String(url).includes('/projects?id=eq.project-1') && options.method === 'PATCH') {
      ...
      if (String(url).includes('/project_events') && options.method === 'POST') {
        return jsonResponse([{ id: 'event-1', project_id: 'project-1', event_type: 'admin_status_updated' }]);
      }
  ...
  const eventCall = calls.find((call) => call.url.includes('/project_events') && call.method === 'POST');
...
test('addAdminProjectNote stores a note and logs an admin event', async () => {
  const calls = [];
  const detail = await addAdminProjectNote('project-1', 'Need alternate vocal ride version.', {
    adminEmail: 'josh@example.com',
    env: { SUPABASE_URL: 'https://project.supabase.co', SUPABASE_SERVICE_ROLE_KEY: 'service-key' },
    fetchImpl: async (url, options = {}) => {
      calls.push({ url: String(url), method: options.method || 'GET', body: options.body ? JSON.parse(options.body) : null });
      ...
      if (String(url).includes('/admin_notes') && options.method === 'POST') {
        return jsonResponse([{ id: 'note-1', project_id: 'project-1', note: 'Need alternate vocal ride version.', created_at: '2026-05-19T10:00:00.000Z' }]);
      }

      if (String(url).includes('/project_events') && options.method === 'POST') {
        return jsonResponse([{ id: 'event-1', project_id: 'project-1', event_type: 'admin_note_added' }]);
      }
      ...
      if (String(url).includes('/payments')) return jsonResponse([]);
      if (String(url).includes('/project_events?project_id=eq.project-1')) {
        return jsonResponse([{ id: 'event-1', project_id: 'project-1', event_type: 'admin_note_added', actor_type: 'admin', message: 'Admin note added.', created_at: '2026-05-19T10:00:00.000Z' }]);
      }
      if (String(url).includes('/email_events')) return jsonResponse([]);
  ...
  const noteCall = calls.find((call) => call.url.includes('/admin_notes') && call.method === 'POST');
...
test('updateAdminProjectDelivery saves a final delivery URL and keeps delivery locked when balance remains', async () => {
  ...
  const detail = await updateAdminProjectDelivery('project-1', {
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (String(url).includes('/project_events') && options.method === 'POST') {
        return jsonResponse([{ id: 'event-1', project_id: 'project-1', event_type: 'admin_delivery_updated' }]);
      }
      ...
      if (String(url).includes('/admin_notes')) return jsonResponse([]);
      if (String(url).includes('/project_events?project_id=eq.project-1')) {
        return jsonResponse([{ id: 'event-1', project_id: 'project-1', event_type: 'admin_delivery_updated', actor_type: 'admin', message: 'Final delivery updated.', created_at: '2026-05-19T11:00:00.000Z' }]);
      }
      if (String(url).includes('/email_events')) return jsonResponse([]);
  ...
  assert.equal(detail.driveLinks.finalDelivery, 'https://drive.test/final-delivery');

  const patchCall = calls.find((call) => call.method === 'PATCH');
  assert.equal(patchCall.body.final_delivery_url, 'https://drive.test/final-delivery');
...
test('updateAdminProjectDelivery unlocks delivery, marks delivered, and logs customer email when balance is clear', async () => {
  ...
  const detail = await updateAdminProjectDelivery('project-1', {
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (String(url).includes('/project_events') && options.method === 'POST') {
        return jsonResponse([{ id: 'event-1', project_id: 'project-1', event_type: 'final_delivery_unlocked' }]);
      }

      if (String(url).includes('/email_events') && options.method === 'POST') {
        return jsonResponse([{ id: 'email-event-1', project_id: 'project-1', email_type: 'final_delivery_unlocked', status: 'sent' }]);
      }
      ...
      if (String(url).includes('/admin_notes')) return jsonResponse([]);
      if (String(url).includes('/project_events?project_id=eq.project-1')) {
        return jsonResponse([{ id: 'event-1', project_id: 'project-1', event_type: 'final_delivery_unlocked', actor_type: 'admin', message: 'Final delivery unlocked for customer access.', created_at: '2026-05-19T11:30:00.000Z' }]);
      }
      if (String(url).includes('/email_events?project_id=eq.project-1')) return jsonResponse([{ id: 'email-event-1', project_id: 'project-1', email_type: 'final_delivery_unlocked', recipient: 'client@example.com', status: 'sent', created_at: '2026-05-19T11:30:00.000Z' }]);
  ...
  assert.equal(sendEmailCall.message.data.finalDeliveryUrl, 'https://drive.test/final-delivery');
});

test('updateAdminProjectDelivery rejects unlock when balance remains', async () => {
  await assert.rejects(
    () => updateAdminProjectDelivery('project-1', {
      unlockDelivery: true,
    }, {
      adminEmail: 'josh@example.com',
      env: { SUPABASE_URL: 'https://project.supabase.co', SUPABASE_SERVICE_ROLE_KEY: 'service-key' },
...
test('allowAdminExtraRevision increments extra revision allowance and logs an admin event', async () => {
  ...
  const detail = await allowAdminExtraRevision('project-1', {
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (String(url).includes('/project_events') && options.method === 'POST') {
        return jsonResponse([{ id: 'event-1', project_id: 'project-1', event_type: 'admin_extra_revision_allowed' }]);
      }
      ...
      if (String(url).includes('/admin_notes')) return jsonResponse([]);
      if (String(url).includes('/project_events?project_id=eq.project-1')) {
        return jsonResponse([{ id: 'event-1', project_id: 'project-1', event_type: 'admin_extra_revision_allowed', actor_type: 'admin', message: 'One extra revision was allowed.', created_at: '2026-05-19T12:00:00.000Z' }]);
      }
      if (String(url).includes('/email_events')) return jsonResponse([]);
...
test('admin project status endpoint updates status for admins', async () => {
  const handler = createAdminProjectsHandler({
    ...
    records: {
      updateAdminProjectStatus: async (projectId, status, options) => ({
        ...
        timeline: [{ eventType: 'admin_status_updated', metadata: { adminEmail: options.adminEmail } }],
      }),
    },
  });
  const res = response();
  ...
  assert.equal(res.body.project.timeline[0].metadata.adminEmail, 'josh@example.com');
});
...
test('admin project notes endpoint adds a note for admins', async () => {
  const handler = createAdminProjectsHandler({
    ...
    records: {
      addAdminProjectNote: async (projectId, note, options) => ({
        ...
        adminNotes: [{ id: 'note-1', note, createdAt: '2026-05-19T10:00:00.000Z' }],
        timeline: [{ eventType: 'admin_note_added', metadata: { adminEmail: options.adminEmail } }],
      }),
    },
  });
  const res = response();
  ...
  await handler({
    ...
    body: { projectId: 'project-1', note: 'Need alternate vocal ride version.' },
  }, res);

  assert.equal(res.statusCode, 200);
  assert.equal(res.body.project.adminNotes[0].note, 'Need alternate vocal ride version.');
  assert.equal(res.body.project.timeline[0].metadata.adminEmail, 'josh@example.com');
});
...
test('admin project delivery endpoint updates the delivery URL for admins', async () => {
  const handler = createAdminProjectsHandler({
    ...
    records: {
      updateAdminProjectDelivery: async (_projectId, update, options) => ({
        project: { id: 'project-1', projectCode: 'DCR-000123', status: 'balance_due', finalDeliveryLocked: true },
        driveLinks: { finalDelivery: update.finalDeliveryUrl },
        timeline: [{ eventType: 'admin_delivery_updated', metadata: { adminEmail: options.adminEmail } }],
      }),
    },
  });
  const res = response();
  ...
  await handler({
    ...
    body: { projectId: 'project-1', finalDeliveryUrl: 'https://drive.test/final-delivery', unlockDelivery: false },
  }, res);

  assert.equal(res.statusCode, 200);
  assert.equal(res.body.project.driveLinks.finalDelivery, 'https://drive.test/final-delivery');
  assert.equal(res.body.project.timeline[0].metadata.adminEmail, 'josh@example.com');
});
  ...
  const handler = createAdminProjectsHandler({
    ...
    records: {
      allowAdminExtraRevision: async (projectId, options) => ({
        project: { id: projectId, projectCode: 'DCR-000123' },
        revisions: { included: 1, used: 1, extraAllowed: 1, remaining: 1, items: [] },
        timeline: [{ eventType: 'admin_extra_revision_allowed', metadata: { adminEmail: options.adminEmail } }],
      }),
    },
  });
  const res = response();
...
test('admin project status endpoint only accepts POST', async () => {
  ...
  await handler({ method: 'GET', headers: {}, url: '/api/admin/projects?action=status&projectId=project-1&status=mixing' }, res);
...
test('admin project notes endpoint only accepts POST', async () => {
  ...
  await handler({ method: 'GET', headers: {}, url: '/api/admin/projects?action=notes&projectId=project-1' }, res);
...
test('admin project delivery endpoint only accepts POST', async () => {
  ...
  await handler({ method: 'GET', headers: {}, url: '/api/admin/projects?action=delivery&projectId=project-1' }, res);
...
test('admin project extra revision endpoint only accepts POST', async () => {
  ...
  await handler({ method: 'GET', headers: {}, url: '/api/admin/projects?action=extra-revision&projectId=project-1' }, res);
