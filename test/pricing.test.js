...
test("catalog exposes approved add-ons", () => {
  ...
  assert.equal(ADD_ONS.rushDelivery, undefined);
  assert.equal(ADD_ONS.consultation, undefined);
});
...
test("add-ons are not discounted", () => {
  ...
  assert.equal(order.discountCents, 2980);
  assert.equal(order.addOnSubtotalCents, 17000);
  assert.equal(order.totalCents, 43820);
});
...
test("deposit is 50 percent for eligible total", () => {
  ...
  assert.equal(order.totalCents, 53730);
  assert.equal(order.amountDueNowCents, 26865);
  assert.equal(order.remainingBalanceCents, 26865);
});
...
test("removed per-project add-ons are rejected", () => {
  assert.throws(
    () =>
      calculateOrder({
        baseServiceId: "mix",
  ...
  assert.throws(
    () =>
      calculateOrder({
        baseServiceId: "mix",
        songCount: 1,
        selectedAddOns: [{ addOnId: "consultation", quantity: 1 }],
        paymentMode: "full",
      }),
    /Unknown add-on/
  );
  ...
  assert.throws(
    () =>
      calculateOrder({
        baseServiceId: "customDeposit",
        songCount: 1,
        selectedAddOns: [{ addOnId: "extraRevision", quantity: 1 }],
        paymentMode: "full",
      }),
