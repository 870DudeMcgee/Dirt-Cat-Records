    ...
    },
    {
      ...
      fetchImpl: async (url, options) => {
        ...
        if (String(url).includes("/payments"))
          return jsonResponse([{ id: "payment-1" }]);
...
test("getFollowUpCandidates queries projects and returns selector candidates", async () => {
  const candidates = await getFollowUpCandidates({
    ...
    fetchImpl: async (url) => {
      ...
      assert.match(decoded, /\/projects\?select=/);
      assert.match(
        decoded,
        /status=in\.\(awaiting_files,quote_sent,finals_ready,balance_due,delivered\)/
      );
      return jsonResponse([
        {
          id: "project-1",
  ...
  assert.equal(candidates[0].projectId, "project-1");
});

test("queueFollowUpJobs queues pending jobs and skips duplicate pending jobs", async () => {
  const calls = [];
  const result = await queueFollowUpJobs(
    [
      {
        projectId: "project-1",
    ...
    ],
    {
      ...
      fetchImpl: async (url, options = {}) => {
        ...
        calls.push({ url: asString, method, body });

        if (asString.includes("/followup_jobs") && method === "POST") {
          if (
            calls.filter(
              (entry) =>
                entry.url.includes("/followup_jobs") && entry.method === "POST"
            ).length === 1
          ) {
            return jsonResponse([{ id: "job-1", status: "pending" }]);
          }
          return {
            ok: false,
            status: 409,
            async text() {
              return 'duplicate key value violates unique constraint "followup_jobs_unique_pending_idx"';
            },
          };
        }

        if (asString.includes("/project_events") && method === "POST") {
          return jsonResponse([{ id: "event-1" }]);
        }
  ...
  assert.equal(result.skipped[0].reason, "duplicate_pending");
  assert.equal(result.failed.length, 0);
  assert.ok(
    calls.some(
      (call) =>
        call.url.includes("/project_events") &&
        call.body.event_type === "followup_job_enqueued"
    )
  );
  assert.ok(
    calls.some(
      (call) =>
        call.url.includes("/project_events") &&
        call.body.event_type === "followup_job_duplicate_skipped"
    )
  );
});

test("listPendingFollowUpJobs qualifies the project relationship for follow-up jobs", async () => {
  const jobs = await listPendingFollowUpJobs({
    now: "2026-05-19T12:00:00.000Z",
    env: {
      SUPABASE_URL: "https://project.supabase.co",
      SUPABASE_SERVICE_ROLE_KEY: "service-key",
    ...
    fetchImpl: async (url) => {
      ...
      assert.match(decoded, /\/followup_jobs\?/);
      assert.match(
        decoded,
        /projects!followup_jobs_project_id_fkey\(id,project_code,project_title,status,balance_due,final_delivery_locked,final_delivery_url\)/
      );
      return jsonResponse([]);
    },
  });
...
test("createAdminQuote stores quote, line items, updates active quote, and logs an admin event", async () => {
    ...
    },
    {
      ...
      fetchImpl: async (url, options = {}) => {
        ...
        if (
          ...
          return jsonResponse([
            { id: "project-1", customer_id: "customer-1", status: "reviewing" },
          ]);
        }
        ...
        if (
          ...
          return jsonResponse([
            { id: "project-1", active_quote_id: "quote-1", status: "quoted" },
          ]);
        }
          ...
          return jsonResponse([
            {
              id: "event-1",
              project_id: "project-1",
              event_type: "admin_quote_created",
            },
          ]);
        }
        throw new Error(`Unexpected URL: ${url}`);
  ...
  const quoteInsert = calls.find(
    (call) => call.url.includes("/quotes") && call.method === "POST"
  );
  assert.equal(quoteInsert.body.final_total_cents, 45000);

  const lineItemInserts = calls.filter(
    (call) => call.url.includes("/quote_line_items") && call.method === "POST"
  );
  assert.equal(lineItemInserts.length, 2);

  const projectPatch = calls.find(
    (call) =>
      call.url.includes("/projects?id=eq.project-1") && call.method === "PATCH"
  );
...
test("sendAdminQuote sends quote email, updates quote and project status, and logs email event", async () => {
  ...
  const quote = await sendAdminQuote("project-1", "quote-1", {
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (
        ...
        return jsonResponse([
          { id: "project-1", active_quote_id: "quote-1", status: "quote_sent" },
        ]);
      }
      if (String(url).includes("/project_events") && method === "POST") {
        return jsonResponse([
  ...
  assert.match(
    sendEmailCall.message.data.quoteUrl,
    /portal\.html\?project=project-1&quote=quote-1/
  );
  ...
  const projectPatch = calls.find(
    (call) =>
      call.url &&
      call.url.includes("/projects?id=eq.project-1") &&
      call.method === "PATCH"
  );
  assert.equal(projectPatch.body.status, "quote_sent");

  const emailEventInsert = calls.find(
    (call) =>
...
test("createAdminQuote can build quote line items from catalog inputs and adjustment", async () => {
  ...
  assert.ok(
    lineItemBodies.some(
      (item) =>
        item.item_type === "adjustment" && item.label === "Loyalty discount"
    )
  );
});
  ...
  const run = await createAutomationTestRun(
    ...
    },
    {
      ...
      fetchImpl: async (url, options) => {
        calls.push({ url: String(url), body: JSON.parse(options.body) });
        return jsonResponse([
          { id: "test-run-1", mode: "simulation", status: "running" },
        ]);
      },
    }
  );
...
test("deleteStudioRecord deletes only cleanup-allowed records", async () => {
  ...
  await assert.rejects(
    () => deleteStudioRecord("customers", "customer-1"),
    /Cleanup cannot delete customers records/
  );
});
