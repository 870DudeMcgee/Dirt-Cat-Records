...
test("getHeader reads PayPal headers case-insensitively", () => {
  assert.equal(
    getHeader({ "paypal-transmission-id": "abc" }, "PAYPAL-TRANSMISSION-ID"),
    "abc"
  );
});
      ...
      purchase_units: [
        {
          ...
          payments: {
            captures: [
              {
                id: "CAPTURE-123",
                amount: { value: "398.00", currency_code: "USD" },
              },
            ],
          },
        },
      ...
      purchase_units: [
        {
          ...
          payments: {
            captures: [
              {
                id: "CAPTURE-123",
                amount: { value: "225.00", currency_code: "USD" },
              },
            ],
          },
        },
...
test("extracts balance payment metadata from custom id on completed capture", () => {
  const metadata = buildOrderMetadata({
    paymentPurpose: "balance",
    projectId: "proj_balance_1",
    amountCents: 22500,
    totalCents: 22500,
  });

  const summary = parseCompletedPaymentEvent({
    event_type: "PAYMENT.CAPTURE.COMPLETED",
    resource: {
  ...
  assert.equal(summary.paypalTxnId, "CAPTURE-BALANCE-1");
  assert.equal(summary.paypalOrderId, "ORDER-BALANCE-1");
  assert.equal(summary.paymentPurpose, "balance");
