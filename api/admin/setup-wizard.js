...
function createSetupWizardHandler(dependencies = {}) {
  ...
  return async function setupWizardHandler(req, res) {
    try {
      ...
      if (action === 'setup') return handleSetup({ req, res, dependencies, env });
      if (action === 'test-runs') return handleTestRuns({ req, res, dependencies, records, env });
      if (action === 'cleanup') return handleCleanup({ req, res, dependencies, records });

      return sendJson(res, 404, { error: 'Admin action not found.' });
    } catch (error) {
      return sendJson(res, error.statusCode || 500, {
        error: error.statusCode ? error.message : 'Unable to complete admin setup request.',
      });
    }
  };
}
