...
function createPortalActionsHandler(dependencies = {}) {
  ...
  return async function portalActionsHandler(req, res) {
    ...
    if (action === 'projects') return handleProjects({ req, res, records, user });
    if (action === 'file-links') return handleFileLinks({ req, res, records, sendEmail, user });
    if (action === 'revisions') return handleRevisions({ req, res, records, sendEmail, env, user });
    if (action === 'approvals') return handleApprovals({ req, res, records, sendEmail, env, user });
...
async function handleProjects({ req, res, records, user }) {
  ...
  try {
    const email = records.normalizeEmail ? records.normalizeEmail(user.email) : recordsDefault.normalizeEmail(user.email);
